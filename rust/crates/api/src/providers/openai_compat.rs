...
impl OpenAiCompatConfig {
    ...
    /// MiniMax API endpoint (MiniMax Chat series models).
    /// Uses the OpenAI-compatible REST shape.
    #[must_use]
    ...
    /// DeepSeek API endpoint (DeepSeek Chat/Coder series models).
    /// Uses the OpenAI-compatible REST shape.
    #[must_use]
...
fn strip_routing_prefix(model: &str) -> &str {
    if let Some(pos) = model.find('/') {
        ...
        if matches!(prefix, "openai" | "xai" | "grok" | "qwen" | "minimax" | "deepseek") {
            &model[pos + 1..]
        } else {
            model
        }
